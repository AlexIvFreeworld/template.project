...
use \Bitrix\Main\Application,
	...
	\Aspro\Allcorp3Medc\Functions\CAsproAllcorp3Admin as Admin,
	CAllcorp3MedcRegionality as Regionality,
	Aspro\Allcorp3Medc\Functions\Extensions,
	\Aspro\Allcorp3Medc\Property\ListWebForms,
	CAllcorp3MedcConst as CConst,
	\Aspro\Allcorp3Medc\Phones\Factory as SolutionPhones;

Loc::loadMessages(__FILE__);

class CAllcorp3Medc{

	use \Aspro\Allcorp3Medc\Traits\Serialize;
	...
	public static function ShowLogo($arParams = array()){
		...
		$arImg = static::unserialize(Option::get(self::moduleID, $logoOptionCode, serialize(array()))) ?: ($isWhite ? static::unserialize(Option::get(self::moduleID, 'LOGO_IMAGE', serialize(array()))) : array());
		...
		if(!$arValues){
			...
			if($arNestedValues && $bNestedParams) //get nested params bd value
			{
				foreach($arNestedValues as $key => $arAllValues)
				{
					...
					{
						$arTmpValues = static::unserialize(Option::get(self::moduleID, 'NESTED_OPTIONS_'.$key.'_'.$key2, serialize(array()), $SITE_ID));
						if($arTmpValues)
						{
			...
			foreach($arValues as $key => $value)
			{
				if(
					...
					$arValue = static::unserialize(Option::get(self::moduleID, $key, serialize(array()), $SITE_ID));
	...
	public static function showAllAdminRows($optionCode, $arTab, $arOption, $module_id, $arPersonTypes, $optionsSiteID, $arDeliveryServices, $arPaySystems, $arCurrency, $arOrderPropertiesByPerson, $bSearchMode){
		if(array_key_exists($optionCode, $arTab["OPTIONS"]) || $arOption["TYPE"] == 'note' || $arOption["TYPE"] == 'includefile')
		{
			...
			elseif($optionCode === "TARIFFS_DEFAULT_PRICE_KEY"){
				$iblockId = Cache::$arIBlocks[$optionsSiteID]['aspro_allcorp3_catalog']['aspro_allcorp3_tarifs'][0];
				if ($iblockId) {
					$arOption['LIST'] = \Aspro\Allcorp3Medc\Property\TariffItem::collectPricesPropsKeys($iblockId);
				}
			}
										...
										if ($arNewOptions) {
											...
											foreach ($arNewOptions as $keyOption => $arNewOption) {
												...
												if (!$arTab['OPTIONS'][$fieldKey]) {
													$arTmpValues = static::unserialize(Option::get(self::moduleID, 'NESTED_OPTIONS_'.$optionCode.'_'.$key, serialize(array()), $optionsSiteID));
	...
	public static function ShowAdminRow($optionCode, $arOption, $arTab, $arControllerOption, $btable = false){
		...
		<?elseif($optionType == "note"):?>
			...
			if ($optionCode == 'GOALS_NOTE') {
				$FORMS_GOALS_LIST = '';
				if (Option::get(self::moduleID, 'USE_BITRIX_FORM', 'Y', $optionsSiteID) === 'Y' && \Bitrix\Main\Loader::includeModule('form')) {
					if ($optionsSiteID) {
					...
					if (
						Cache::$arIBlocks[$optionsSiteID]['aspro_' . static::themesSolutionName . '_form']
						&& is_array(Cache::$arIBlocks[$optionsSiteID]['aspro_' . static::themesSolutionName . '_form'])
					) {
						foreach (Cache::$arIBlocks[$optionsSiteID]['aspro_' . static::themesSolutionName . '_form'] as $arIDs) {
							if ($arIDs && is_array($arIDs)) {
								foreach ($arIDs as $iblockID) {
									if (Cache::$arIBlocksInfo && Cache::$arIBlocksInfo[$iblockID] && is_array(Cache::$arIBlocksInfo[$iblockID])) {
										$FORMS_GOALS_LIST .= Cache::$arIBlocksInfo[$iblockID]['NAME'] . ' - <i>goal_webform_success_' . $iblockID . '</i><br />';
									}
								}
							}
						}
			...
			<?endif;?>
				...
				<?elseif($optionType == "text" || $optionType == "password"):?>
					...
					<?elseif(strpos($optionCode, 'HEADER_PHONES_array_PHONE_ICON') !== false):?>
						<div class="iconset_value" data-code="header_phones" title="<?=htmlspecialcharsbx($arOption['TITLE'])?>"><div class="iconset_value_wrap"><?=\Aspro\Allcorp3Medc\Iconset::showIcon($optionVal)?></div><input type="hidden" value="<?=htmlspecialcharsbx($optionVal)?>" name="<?=htmlspecialcharsbx($optionCode)."_".$optionsSiteID?>"></div>
				...
				<?elseif($optionType == "file"):?>
					<?$val = static::unserialize(Option::get(self::moduleID, $optionCode, serialize(array()), $optionsSiteID));
	...
	public static function CheckDetailPageUrlInMultilevel(&$arResult){
		if($arResult['ITEMS']){
			...
			$arItems = Cache::CIBLockElement_GetList(array('CACHE' => array('TAG' => Cache::GetIBlockCacheTag($arResult['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N')), array('ID' => $arItemsIDs), false, false, array('ID', 'IBLOCK_SECTION_ID', 'DETAIL_PAGE_URL'));
	...
	public static function CopyFaviconToSiteDir($arValue, $siteID = ''){
		if(($siteID)){
			if(is_string($arValue) && $arValue) {
				$arValue = static::unserialize($arValue);
			}
   ...
   protected static function IsComponentTemplateHasModuleElementsPageBlocksParam($templateName, $arExtParams = array()){
	...
	/**
	 * @deprecated use \Aspro\Allcorp3Medc\Phones\Factory::create('mobile_menu')
	 */
	public static function ShowMobileMenuPhones($arOptions = [])
	{
		$obPhones = SolutionPhones::create('mobile_menu', $arOptions);
		$obPhones->show();
	}

	/**
	 * @deprecated use \Aspro\Allcorp3Medc\Phones\Factory::create('mobile')
	 */
	public static function ShowHeaderMobilePhones(array $arOptions = []): void
	{
		$obPhones = SolutionPhones::create('mobile', $arOptions);
		$obPhones->show();
	}

	/**
	 * @deprecated use \Aspro\Allcorp3Medc\Phones\Factory::create('common')
	 */
	public static function ShowHeaderPhones(
		$class = '', $icon = '', $only_icon = false, $dropdownTop = false, $bDropdownCallback = 'Y', $bDropdownEmail = 'Y', 
		$bDropdownSocial = 'Y', $bDropdownAddress = 'Y', $bDropdownSchedule = 'Y'
	): void
	{
		$obPhones = SolutionPhones::create('common', [
			'CLASS' => $class, 'ICON' => $icon, 'SHOW_ONLY_ICON' => $only_icon, 
			'DROPDOWN_TOP' => $dropdownTop, 'IS_DROPDOWN_CALLBACK' => $bDropdownCallback === 'Y', 
			'IS_DROPDOWN_EMAIL' => $bDropdownEmail === 'Y', 'IS_DROPDOWN_SOCIAL' => $bDropdownSocial === 'Y', 
			'IS_DROPDOWN_ADDRESS' => $bDropdownAddress === 'Y', 'IS_DROPDOWN_SCHEDULE' => $bDropdownSchedule === 'Y'
		]);
		$obPhones->show();
	}

	/**
	 * @deprecated use \Aspro\Allcorp3Medc\Phones\Factory::create('contacts')
	 */
	public static function showContactPhones($txt = '', $wrapTable = true, $class = '', $icon = 'Phone_black.svg')
	{
		$obPhones = SolutionPhones::create('contacts', [
			'LABEL' => $txt,
			'CLASS' => $class,
		]);
		$obPhones->show();
	...
	public static function showContactAddr($txt = '', $wrapTable = true, $class = '', $icon = 'Addres_black.svg'){
		...
		$bRegionContact = (\Bitrix\Main\Config\Option::get(self::moduleID, 'SHOW_REGION_CONTACT', 'N') == 'Y');
		$bFromRegion = $arRegion && $bRegionContact;
		$bAddr = ($bFromRegion ? !empty($arRegion['PROPERTY_ADDRESS_VALUE']['TEXT']): self::checkContentFile(SITE_DIR.'include/contacts-site-address.php'));
		?>
		<?if($arRegion):?>
	...
	public static function showContactSchedule($txt = '', $wrapTable = true, $class = '', $icon = '', $subclass = ''){
		...
		$bRegionContact = (\Bitrix\Main\Config\Option::get(self::moduleID, 'SHOW_REGION_CONTACT', 'N') == 'Y');
		$bFromRegion = $arRegion && $bRegionContact;
		$regionText = is_array($arRegion['PROPERTY_SHCEDULE_VALUE']) && isset($arRegion['PROPERTY_SHCEDULE_VALUE']['TEXT']) 
			? $arRegion['PROPERTY_SHCEDULE_VALUE']['TEXT'] 
			: $arRegion['PROPERTY_SHCEDULE_VALUE'];
		$bContent = ($bFromRegion ? !!$regionText : self::checkContentFile(SITE_DIR.'include/contacts-site-schedule.php'));
		?>
		<?if($arRegion):?>
			<?$frame = new \Bitrix\Main\Page\FrameHelper('header-allcaddr-block'.$iCalledID);?>
			<?$frame->begin();?>
	...
	public static function showContactDesc(){
		...
		$bRegionContact = (\Bitrix\Main\Config\Option::get(self::moduleID, 'SHOW_REGION_CONTACT', 'N') == 'Y');
		$bFromRegion = $arRegion && $bRegionContact;
		$bDesc = ($bFromRegion ? !empty($arRegion['PROPERTY_REGION_TAG_CONTACT_TEXT_VALUE']['TEXT']) : self::checkContentFile(SITE_DIR.'include/contacts-site-about.php'));
		?>
		<?if($arRegion):?>
	...
	public static function getDataItem($el, $toJSON = true){
		...
		if ($el['NO_IMAGE'] && !$dataItem['PREVIEW_PICTURE'] && !$dataItem['DETAIL_PICTURE']) {
			$dataItem['PREVIEW_PICTURE'] = $el['NO_IMAGE']['ID'];
		}
	...
	public static function checkElementsIdsInRegion(&$arIds, $iblockId = false, $regionId = false){
		...
		if($bFilterItem && $arIds){
			...
			if($iblockId){
				...
				if($regionId){
					if($arSectionsIds = self::getSectionsIds_NotInRegion($iblockId, $regionId)){
	...
	/**
	 * Draw form field for webform
	 * 
	 * @deprecated use \Aspro\Allcorp3Medc\Form\Field\Webform::draw()
	 */
	public static function drawFormField($FIELD_SID, $arQuestion, $type = 'POPUP', $arParams = array(), $arOptions = []) {
		...
		// column counters block
		static $totalColumnsInRow, $currentFieldIndex;
		if (!isset($totalColumnsInRow)) {
			$totalColumnsInRow = 0;
		}
		if (!isset($currentFieldIndex)) {
			$currentFieldIndex = 1;
		}
		preg_match('/\bcol-(\d+)\b/', $arQuestion['STRUCTURE'][0]['FIELD_PARAM'], $currentColumnSize);
		$currentColumnSize = (isset($currentColumnSize[1]) && $currentColumnSize[1] <= 12) ? $currentColumnSize[1] : 12;
		?>
		...
		<?else:?>
			<?
			$bRowOverflow = ($totalColumnsInRow + $currentColumnSize) > 12;
			if ($bRowOverflow) { // close previous row
				echo "</div>";
			}
			if (!$totalColumnsInRow || $bRowOverflow) { // open new row
				echo "<div class='row' data-SID='".$FIELD_SID."'>";
			}
			...
			?>
			<div class="col-md-<?=$currentColumnSize;?><?=(in_array($arQuestion['STRUCTURE'][0]['FIELD_TYPE'], array("checkbox", "radio")) ? ' style_check bx_filter' : '');?>">
				<div class="form-group<?=(!in_array($arQuestion['STRUCTURE'][0]['FIELD_TYPE'], array("file", "image", "checkbox", "radio", "multiselect", "date1")) ? ' fill-animate' : '');?>">
					<label class="font_13 color_999" for="<?=$type.'_'.$FIELD_SID?>">
						<span><?=$arQuestion["CAPTION"]?><?=($arQuestion["REQUIRED"] == "Y" ? '&nbsp;<span class="required-star">*</span>' : '')?></span>
					</label>
					<div class="input<?=($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] === 'date' ? ' dates' : '');?>">
						...
						if (isset($arResult) && is_array($arResult["FORM_ERRORS"]) && array_key_exists($FIELD_SID, $arResult['FORM_ERRORS'])) {
			...
			<?
			$totalColumnsInRow += $currentColumnSize;
			$lastQuestionField = array_key_exists('COUNT_FIELDS_QUESTION', $arOptions) && $arOptions["COUNT_FIELDS_QUESTION"] === ($currentFieldIndex++);
			if ($totalColumnsInRow >= 12 || $lastQuestionField) {
				$totalColumnsInRow = 0;
				echo "</div>";
			}
			?>
	...
	public static function get_banners_position($position){
		$arTheme = self::GetFrontParametrsValues(SITE_ID);
		$filterName = 'arDopBannerFilter_'.$position;

		if ($arTheme["ADV_".$position] == 'Y') {
			global $APPLICATION, $arRegion;
			
			if($arTheme['USE_REGIONALITY'] === 'Y'){
				if(!empty($arRegion['ID']) && $arTheme['REGIONALITY_FILTER_ITEM'] === 'Y'){
