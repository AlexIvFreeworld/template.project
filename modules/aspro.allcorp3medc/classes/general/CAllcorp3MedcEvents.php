...
use \Bitrix\Main\Localization\Loc,
	...
	Aspro\Allcorp3Medc\CRM,
	CAllcorp3Medc as Solution,
	CAllcorp3MedcCache as Cache,
	CAllcorp3MedcRegionality as Regionality,
	Aspro\Allcorp3Medc\Functions\CAsproAllcorp3ReCaptcha as ReCaptcha,
	Aspro\Allcorp3Medc\Functions\CAsproAllcorp3 as Functions,
	Aspro\Allcorp3Medc\Functions\ThematicParameters as ThematicParameters,
	Aspro\Allcorp3Medc\Notice;

Loc::loadMessages(__FILE__);

class CAllcorp3MedcEvents{
	...
	public static function OnEndBufferContentHandler(&$content)
	{
		...
		{			
			...
			// lazyload
			global $arTheme;
			if(isset($GLOBALS['_USE_LAZY_LOAD_'.mb_strtoupper(Solution::themesSolutionName).'_']) && $GLOBALS['_USE_LAZY_LOAD_'.mb_strtoupper(Solution::themesSolutionName).'_']){
	...
	public static function OnAfterIBlockElementAddHandler($arFields){
		...
		if ($arFields['PROPERTY_VALUES']) {
			...
			foreach (GetModuleEvents(Solution::moduleID, 'OnAsproAfterIBlockElementAddProps', true) as $arEvent)
				ExecuteModuleEventEx($arEvent, array(&$PROP, $arFields));
		}
		$arData = [
			"PROPERTY_VALUES"=> $PROP,
			"NAME"=> $arFields['PROPERTY_VALUES']['NAME'],
			"PREVIEW_TEXT"=> $arFields['PROPERTY_VALUES']['MESSAGE']['VALUE']['TEXT'],
			"DATE_ACTIVE_FROM" =>  $arFields["DATE_ACTIVE_FROM"] ?: ConvertTimeStamp(time(), "FULL"),
		];
