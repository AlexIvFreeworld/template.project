<?
// Cache functions
if(!class_exists('CAllcorp3MedcCache')){
	class CAllcorp3MedcCache {
		...
		public static function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3MedcCache/sale/CSaleBasket_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		public static function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3MedcCache/catalog/CCatalogStore_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		public static function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3MedcCache/forum/CForumMessage_GetListEx/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		private static function _InitCacheParams($moduleName, $functionName, $arCache){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3MedcCache/".$moduleName."/".$functionName."/".$cacheTag."/";
			}
	...
	if(CAllcorp3MedcCache::$arIBlocks === NULL){
		CAllcorp3MedcCache::$bEnabled = COption::GetOptionString('main', 'component_managed_cache_on', 'Y') === 'Y';
		$arIBlocksTmp = CAllcorp3MedcCache::CIBlock_GetList(array("CACHE" => array("TAG" => "iblocks")), array("ACTIVE" => "Y", "CHECK_PERMISSIONS" => "N"));
		CAllcorp3MedcCache::$arIBlocks = CAllcorp3MedcCache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("LID", "IBLOCK_TYPE_ID", "CODE"), "MULTI" => "Y", "RESULT" => array("ID")));
		CAllcorp3MedcCache::$arIBlocksInfo = CAllcorp3MedcCache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("ID")));
	}
}