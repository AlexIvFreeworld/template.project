...
if($arParams['USE_REGION_DATA'] == 'Y' && TSolution::GetFrontParametrValue('SHOW_REGION_CONTACT') == "Y" &&  $arRegion && $arRegion["PROPERTY_REGION_TAG_YANDEX_MAP_VALUE"])
{
	...
	$phones = '';
	if($arRegion['PHONES']) {
		foreach ($arRegion['PHONES'] as $arPhoneItem) {
			$phones .= '<div class="value"><a class="dark_link" rel= "nofollow" href="tel:'.str_replace(array(' ', ',', '-', '(', ')'), '', $arPhoneItem['PHONE']).'">'.$arPhoneItem['PHONE'].'</a></div>';
		}
	}
