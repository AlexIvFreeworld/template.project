				...
				<div class="doc-list-inner__list  grid-list <?= $listClasses ?>">
					...
					<? foreach ($arSection['ITEMS'] as $i => $arItem) : ?>
						...
						if (isset($arItem['PROPERTIES']['DOCUMENT']) && $arItem['PROPERTIES']['DOCUMENT']['VALUE']) {
							$arDocFile = CAllcorp3Medc::GetFileInfo($arItem['PROPERTIES']['DOCUMENT']['VALUE']);
										...
										<? if ($arDocFile) : ?>
											<? if ($bDocImage) : ?>
												<a class="doc-list-inner__icon-preview-image doc-list-inner__link-file  fancy fill-theme-parent"
												   data-caption="<?= htmlspecialchars($arItem['NAME']) ?>"
												   href="<?= $arDocFile['SRC'] ?>">
													<?= CAllcorp3Medc::showIconSvg('image-preview fill-theme-target', SITE_TEMPLATE_PATH . '/images/svg/preview_image.svg'); ?>
												</a>
											<? else : ?>
												<a class="doc-list-inner__icon-preview-image doc-list-inner__link-file  fill-theme-parent"
												   target="_blank"
												   href="<?= $arDocFile['SRC'] ?>">
													<?= CAllcorp3Medc::showIconSvg('image-preview fill-theme-target', SITE_TEMPLATE_PATH . '/images/svg/file_download.svg'); ?>
												</a>
											<? endif ?>
										<? endif ?>
									</div>
