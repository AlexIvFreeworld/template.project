	...
	for($index = 0; $index < $cnt; ++$index){
		...
		if ($NextSectionID) {
			...
			if ($bShowCatalogSubsections) {
				$arSubSections = CAllcorp3Medc::getChainNeighbors($NextSectionID, $arItem['LINK']);
			}
		}
		if($index){
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			...
			if($arSubSections){
				...
				if ($bShowMobileArrow) {
					$strReturn .= CAllcorp3Medc::showIconSvg('colored_theme_hover_bg-el-svg', SITE_TEMPLATE_PATH.'/images/svg/catalog/arrow_breadcrumbs.svg');
				}
				$strReturn .=($arSubSections ? '<span itemprop="name" class="breadcrumbs__item-name font_13">'.$title.'</span><span class="breadcrumbs__arrow-down '.(!$bLast ? 'colored_theme_hover_bg-el-svg' : '').'">'.CAllcorp3Medc::showIconSvg("arrow", SITE_TEMPLATE_PATH."/images/svg/trianglearrow_down.svg").'</span>' : '<span>'.$title.'</span>');
