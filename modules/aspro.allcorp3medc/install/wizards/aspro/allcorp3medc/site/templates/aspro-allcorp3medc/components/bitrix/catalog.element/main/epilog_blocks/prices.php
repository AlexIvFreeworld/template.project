...
<?if($templateData['PRICES']['VALUE'] && $templateData['PRICES']['IBLOCK_ID']):?>
    <?if(!isset($html_prices)):?>
        ...
        <?ob_start();?>
        <?$bCheckAjaxBlockPrice = TSolution::checkRequestBlock("price-list-inner");?>
        <?$isAjaxPrice = (TSolution::checkAjaxRequest() && $bCheckAjaxBlockPrice ) ? 'Y' : 'N'; ?>
        <?$APPLICATION->IncludeComponent(
            "bitrix:news.list",
            "price-list",
            array(
                "IBLOCK_TYPE" => TSolution::partnerName."_".TSolution::themesSolutionName."_catalog",
                "IBLOCK_ID" => $templateData['PRICES']['IBLOCK_ID'],
                "NEWS_COUNT" => TSolution::GetFrontParametrValue('PRICE_LINK_COUNT') ?? "20",
                "SORT_BY1" => "SORT",
                "SORT_ORDER1" => "ASC",
                "SORT_BY2" => "ID",
                "SORT_ORDER2" => "DESC",
                ...
                "IS_AJAX" => TSolution::checkAjaxRequest() && $bCheckAjaxBlockPrice,
                "NAME_SIZE" => "18",
                "SUBTITLE" => "",
                "SHOW_PREVIEW_TEXT" => "Y",
            ),
