...
foreach ($arResult['DISPLAY_PROPERTIES'] as $propertyCode => $property) {
	...
	elseif($propertyCode === 'LINK_ADDRESS' && $property['VALUE']){
		...
		$dbRes = CIBlockElement::GetList(array(), array('IBLOCK_ID' => CAllcorp3MedcCache::$arIBlocks[SITE_ID]['aspro_allcorp3medc_content']['aspro_allcorp3medc_contact'][0], 'ID' => $property['VALUE'], 'ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y', 'GLOBAL_ACTIVE' => 'Y'), false, false, array('ID', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'NAME', 'DETAIL_PAGE_URL'));
		...
		if($arResult['ADDRESS']['IBLOCK_SECTION_ID']){
			$arSectionAddress = CAllcorp3MedcCache::CIblockSection_GetList(array("CACHE" => array("TAG" => CAllcorp3MedcCache::$arIBlocks[SITE_ID]['aspro_allcorp3medc_content']['aspro_allcorp3medc_contact'][0]), 'MULTI' => 'N'), array('IBLOCK_ID' => CAllcorp3MedcCache::$arIBlocks[SITE_ID]['aspro_allcorp3medc_content']['aspro_allcorp3medc_contact'][0], 'ID' => $arResult['ADDRESS']['IBLOCK_SECTION_ID']));
			$arParentSectionAddress = CAllcorp3MedcCache::CIblockSection_GetList(array("CACHE" => array("TAG" => CAllcorp3MedcCache::$arIBlocks[SITE_ID]['aspro_allcorp3medc_content']['aspro_allcorp3medc_contact'][0]), 'MULTI' => 'N'), array('IBLOCK_ID' => CAllcorp3MedcCache::$arIBlocks[SITE_ID]['aspro_allcorp3medc_content']['aspro_allcorp3medc_contact'][0], "<=LEFT_BORDER" => $arSectionAddress[0]["LEFT_MARGIN"], ">=RIGHT_BORDER" => $arSectionAddress[0]["RIGHT_MARGIN"], "DEPTH_LEVEL" => 1), false, array('NAME'));
...
if($arResult['DISPLAY_PROPERTIES']){
	$arResult['CHARACTERISTICS'] = CAllcorp3Medc::PrepareItemProps($arResult['DISPLAY_PROPERTIES']);
...
if ($arParams['DISPLAY_PICTURE'] != "N") {
	...
	CAllcorp3Medc::getFieldImageData($arResult, [$pictureField]);
