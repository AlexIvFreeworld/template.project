...
<?if($templateData['SCHEDULE']):?>
    <?if($isAjaxStaff):?>
        ...
        $serviceIblock = \Bitrix\Main\Config\Option::get(TSolution::moduleID, 'SERVICES_IBLOCK_ID', \CAllcorp3MedcCache::$arIBlocks[$site_id]['aspro_allcorp3medc_content']['aspro_allcorp3medc_services'][0], $site_id);
        $contactIblock = \Bitrix\Main\Config\Option::get(TSolution::moduleID, 'CONTACT_IBLOCK_ID', \CAllcorp3MedcCache::$arIBlocks[$site_id]['aspro_allcorp3medc_content']['aspro_allcorp3medc_contact'][0], $site_id);
        ...
        if(!empty($arrShops)){
            $arItemsShops = \CAllcorp3MedcCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => \CAllcorp3MedcCache::GetIBlockCacheTag($contactIblock))), array('IBLOCK_ID' => $contactIblock, 'ACTIVE' => 'Y', 'ID' => $arrShops), false, false, array('ID', 'NAME', 'PROPERTY_ADDRESS', 'PROPERTY_MAP', 'PROPERTY_PHONE', 'PROPERTY_EMAIL', 'PROPERTY_METRO', 'PROPERTY_SCHEDULE', 'DETAIL_PAGE_URL'));
        ...
        if(!empty($serviceIblock)){
            $arItemsServ = \CAllcorp3MedcCache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => \CAllcorp3MedcCache::GetIBlockCacheTag($serviceIblock))), array('IBLOCK_ID' => $serviceIblock, 'ACTIVE' => 'Y', 'ID' => $arrServ), false, false, array('ID', 'NAME', 'DETAIL_PAGE_URL'));
