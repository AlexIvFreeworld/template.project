...
if($arResult['QUESTIONS']['SPECIALIST']){
	...
	if($arStaffs){
		...
		$arResult['QUESTIONS']['SPECIALIST']['HTML_CODE'] = '<div class="wrap"><select id="'.$arResult['QUESTIONS']['SPECIALIST']['CODE'].'" class="form-control '.$required.'" name="form_text_'.$arResult['QUESTIONS']['SPECIALIST']['STRUCTURE'][0]['ID'].'">'.$strValues.'</select></div>';
	}
}
...
if(
	...
	if($arSpecializations){
        ...
        $arResult['QUESTIONS']['SPECIALIZATION']['HTML_CODE'] = '<div class="wrap"><select id="SPECIALIZATION" class="form-control'.$required.'" name="form_text_'.$arResult['QUESTIONS']['SPECIALIZATION']['STRUCTURE'][0]['ID'].'">'.$strValues.'</select></div>';
	}
}
else{
