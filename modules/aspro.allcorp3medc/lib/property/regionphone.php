...
class RegionPhone{
	public static function OnIBlockPropertyBuildList(){
		return array(
			'PROPERTY_TYPE' => 'S',
			'USER_TYPE' => 'SAsproAllcorp3RegionPhone',
			'DESCRIPTION' => Loc::getMessage('ALLCORP3_REGION_PHONE_PROP_TITLE'),
				...
				<div class="inner_wrapper">
					<div class="inner">
						<div class="value_wrapper">
							<input type="text" name="<?=$phoneName?>" value="<?=htmlspecialcharsbx($phone)?>" maxlength="255" placeholder="<?=htmlspecialcharsbx(Loc::getMessage('ALLCORP3_REGION_PHONE_PHONE_TITLE'))?>" title="<?=htmlspecialcharsbx(Loc::getMessage('ALLCORP3_REGION_PHONE_PHONE_TITLE'))?>" />
						</div>
					</div>
					<div class="inner">
						<div class="value_wrapper">
							<input type="text" name="<?=$hrefName?>" value="<?=htmlspecialcharsbx($href)?>" maxlength="255" placeholder="<?=htmlspecialcharsbx(Loc::getMessage('ALLCORP3_REGION_PHONE_HREF_TITLE'))?>" title="<?=htmlspecialcharsbx(Loc::getMessage('ALLCORP3_REGION_PHONE_HREF_TITLE'))?>" />
						</div>
					</div><br />
					<div class="inner">
						<div class="value_wrapper">
							<div class="iconset_value" data-code="header_phones" title="<?=Loc::getMessage('ALLCORP3_REGION_PHONE_ICON_TITLE')?>"><div class="iconset_value_wrap"><?=Iconset::showIcon($icon)?></div><input type="hidden" value="<?=htmlspecialcharsbx($icon)?>" name="<?=$iconName?>"></div>
						</div>
					</div>
					<div class="inner">
						<div class="value_wrapper">
							<?if($bWithDescription):?>
								<input type="text" name="<?=$descriptionName?>" value="<?=htmlspecialcharsbx($description)?>" maxlength="255" placeholder="<?=htmlspecialcharsbx(Loc::getMessage('ALLCORP3_REGION_PHONE_DESCRIPTION_TITLE'))?>" title="<?=htmlspecialcharsbx(Loc::getMessage('ALLCORP3_REGION_PHONE_DESCRIPTION_TITLE'))?>" <?=($bEditProperty ? 'readonly disabled' : '')?> />
							<?else:?>
								<div class="description_disabled_note"><?=Loc::getMessage('ALLCORP3_REGION_PHONE_DESCRIPTION_FIELD_DISABLED_NOTE');?></div>
							<?endif;?>
						</div>
					</div>
					<?if(!$bEditProperty):?>
						<div class="remove" title="<?=Loc::getMessage('ALLCORP3_REGION_PHONE_DELETE_TITLE')?>"></div>
						<div class="drag" title="<?=Loc::getMessage('ALLCORP3_REGION_PHONE_DRAG_TITLE')?>"></div>
					<?endif;?>
				</div>
			</div>
		</div>
