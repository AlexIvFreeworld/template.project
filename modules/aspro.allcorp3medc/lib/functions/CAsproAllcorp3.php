...
use \Bitrix\Main\Application,
	...
	\Aspro\Allcorp3Medc\Functions\CAsproAllcorp3CRM as CRM,
	\CAllcorp3MedcCache as Cache,
	\Aspro\Allcorp3Medc\Eyed as Eyed,
	\CAllcorp3MedcCondition as Condition,
...
class CAsproAllcorp3
{
	...
	public static function addFormResultToIBlock($WEB_FORM_ID, $RESULT_ID){
		...
		if(!$bAdminSection)
		{
			...
			if($arForm && strpos($arForm['SID'], str_replace('.', '_', self::MODULE_ID).'_feedback') !== false)
			{
				...
				{
					$reviewsIBlockID = Cache::$arIBlocks[SITE_ID]["aspro_".Solution::themesSolutionName."_content"]["aspro_".Solution::themesSolutionName."_reviews"][0];
					if (!$reviewsIBlockID) return;
					...
					foreach(GetModuleEvents(Solution::moduleID, 'OnAsproAddFormResultToIBlockField', true) as $arEvent) // event for manipulation store quantity block
						ExecuteModuleEventEx($arEvent, array($arrAnswersVarname, $RESULT_ID, &$PROP));
						...
						<div class="<?=$options['NO_ICON'] ? '' : 'icon-block--with_icon'?> <?=$options['ONLY_ICON'] ? 'icon-block--only_icon' : ''?>">
							<div class="phones">
								...
								<div class="phones__phones-wrapper">
									<?
									$arPhonesOptions = [
										'CLASS' => 'phones__inner--big', 
										'ICON' => $options['ONLY_ICON'] ? 'Phone_big.svg' : 'Phone_black.svg', 
										'SHOW_ONLY_ICON' => $options['ONLY_ICON'], 
										'DROPDOWN_TOP' => $options['DROPDOWN_TOP'], 
										'IS_DROPDOWN_CALLBACK' => $options['DROPDOWN_CALLBACK'] === 'Y', 
										'IS_DROPDOWN_EMAIL' => $options['DROPDOWN_EMAIL'] === 'Y', 
										'IS_DROPDOWN_SOCIAL' => $options['DROPDOWN_SOCIAL'] === 'Y', 
										'IS_DROPDOWN_ADDRESS' => $options['DROPDOWN_ADDRESS'] === 'Y', 
										'IS_DROPDOWN_SCHEDULE' => $options['DROPDOWN_SCHEDULE'] === 'Y',
									];
									$obPhones = SolutionPhones::create('common', $arPhonesOptions);
									$obPhones->show();
									?>
								</div>
					...
					<div class="icon-block--with_icon icon-block--only_icon">
						<div class="phones">
							<div class="phones__phones-wrapper">
								...
								$obPhones = SolutionPhones::create('mobile', $arPhonesOptions);
								$obPhones->show();
								?>
							</div>
						</div>
					...
					<div class="mobilemenu__menu mobilemenu__menu--contacts">
						<ul class="mobilemenu__menu-list">
							<?if($options['PHONES']):?>
								...
								$obPhones = SolutionPhones::create('mobile_menu', $arPhonesOptions);
								$obPhones->show();
								?>
							<?endif;?>
	...
	public static function showBasketButton($arOptions = [])
	{
		...
		$sOneClickBuyText = Option::get(self::MODULE_ID, 'EXPRESSION_ONE_CLICK_BUY_TEXT', GetMessage('ONE_CLICK_BUY'), SITE_ID);
		...
		$arConfig['ONE_CLICK_BUY'] =  ($arConfig['ONE_CLICK_BUY'] === true || $arConfig['ONE_CLICK_BUY'] === 'true') && $sOneClickBuyText;
			...
			<?if($bOrderButton):?>
				...
				<?if(!$bOrderViewBasket || $bOrderViewBasket === 'false'):?>
					<span class="buy_block btn-actions__inner">
						<span class="buttons">
							<span class="btn btn-default <?=$arConfig['BTN_CLASS']?> <?=$arConfig['BTN_CLASS_MORE']?> animate-load" data-event="jqm" data-param-id="<?=Solution::getFormID($arConfig['ORDER_FORM_ID']);?>" data-autoload-product="<?=Solution::formatJsName($arItem["NAME"]);?>" data-autoload-service="<?=Solution::formatJsName($arItem["NAME"]);?>" data-autoload-project="<?=Solution::formatJsName($arItem["NAME"]);?>" data-autoload-news="<?=Solution::formatJsName($arItem["NAME"]);?>" data-autoload-sale="<?=Solution::formatJsName($arItem["NAME"]);?>" data-name="order_product_<?=$arItem['ID'];?>">
								<?=$sOrderText;?>
							</span>
						</span>
							...
							<?else:?>
								<div class="info-buttons <?=($arConfig['ONE_CLICK_BUY'] || $arConfig['QUESTION_BTN'] ? 'flexbox flexbox--direction-row' : '')?>">
									<?if ($arConfig['ONE_CLICK_BUY']):?>
										...
										$currency = $arItem['DISPLAY_PROPERTIES']['PRICE_CURRENCY']['VALUE'] ?? $arItem['PROPERTIES']['PRICE_CURRENCY']['VALUE'];
										?>
										<div class="info-buttons__item info-buttons__item--ocb flex-1">
											...
											data-autoload-product_price="<?=self::replaceCurrencyInPrice($price, $currency)?>"
											>
												<span>
												<?=$sOneClickBuyText;?>
												</span>
	...
	*/
	...
	public static function getItemMapHtml($arOptions)
	{
		...
		foreach(GetModuleEvents(self::MODULE_ID, 'onGetItemMapHtml', true) as $arEvent) // event for manipulation map item block
			ExecuteModuleEventEx($arEvent, array($arOptions, $html, $btnClass));
		...
		if($arItem):?>
			<?ob_start();?>
				<div class="side-icons js-replace-icons <?=$arConfig['DOP_CLASS'];?>">
					...
					<?endif;?>
					<?if ($arParams['SHOW_ONE_CLICK_BUY'] != 'N' && $bOrderButton && $bOrderViewBasket && $sOneClickBuyText):?>
						<?
						$price = ($arItem['DISPLAY_PROPERTIES']['PRICE']['VALUE'] ? $arItem['DISPLAY_PROPERTIES']['PRICE']['VALUE'] : $arItem['PROPERTIES']['PRICE']['VALUE']);
						$currency = $arItem['DISPLAY_PROPERTIES']['PRICE_CURRENCY']['VALUE'] ?? $arItem['PROPERTIES']['PRICE_CURRENCY']['VALUE'];
						?>
						<div class="side-icons__item side-icons__item--ocb bordered rounded-4">
							<a href="javascript:void(0)" rel="nofollow" class="ocb" data-param-id="<?=Solution::getFormID("aspro_".Solution::themesSolutionName."_quick_buy");?>" data-name="ocb" data-event="jqm" title="<?=$sOneClickBuyText?>" data-autoload-product_name="<?=Solution::formatJsName($arItem['NAME'])?>" data-autoload-product_price="<?=self::replaceCurrencyInPrice($price, $currency)?>">
	...
	public static function showImg($arOptions = [])
	{
		...
		<?if($arItem):?>
			...
			<?endif;?>
				...
				<?else:?>
					<?\Aspro\Allcorp3Medc\Product\Image::showNoImage([
						'SRC' => $arItem['NO_IMAGE']['SRC'] ?? '',
						'ADDITIONAL_CLASS' => $dopClassImg,
						'ALT' => $a_alt,
						'TITLE' => $a_title,
					]);?>
				<?endif;?>
					...
					<?endif;?>
						<span class="section-gallery-wrapper js-replace-gallery flexbox">
							<?foreach($arItem[$key] as $i => $arGalleryItem):?>
								...
								?>
								<?if($arConfig['USE_FANCY']):?>
									<a href="<?=$arGalleryItem['SRC'];?>" class="section-gallery-wrapper__item<?=(!$i ? ' active' : '');?> fancy" data-fancybox="gallery <?=$arItem['ID']?>">
								<?else:?>
		...
		foreach($arImages as $i => $arImage){
			$arImages[$i] = array_merge(
				$arImage, array(
					...
					"SMALL" => \CFile::ResizeImageGet($arImage["ID"], array("width" => 520, "height" => 520), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
								...
								switch ($arItem['TYPE']) {
									...
									default:?>
										...
										<?else:?>
											<span
										<?endif;?>
											class="bottom-icons-panel__content-link fill-theme-parent<?=($arItem['TYPE'] ? ' bottom-icons-panel__content-link--'.$arItem['TYPE'] : '')?><?=($bActive ? ' bottom-icons-panel__content-link--active' : ' dark_link')?> bottom-icons-panel__content-link--with-counter" 
