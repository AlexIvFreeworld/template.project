...
$arAllOptions = array(
	array("link_elec_cat", GetMessage("IALEX_IMPORTXML_LINK_ELEC_CAT"), "https://www.elec.ru/help/", array("text", 50)),
	array("link_elec_price", GetMessage("IALEX_IMPORTXML_LINK_ELEC_PRICE"), "https://www.elec.ru/help/market/xmlprice.html", array("text", 50)),
);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST['Update'] || $_POST['Apply'] || $_POST['RestoreDefaults']) > 0 && check_bitrix_sessid()) {
	if ($_POST['RestoreDefaults'] <> '') {
		...
		foreach ($arDefValues as $key => $value) {
			COption::RemoveOption("ialex.importxml", $key);
		}
	} else {
		foreach ($arAllOptions as $arOption) {
			...
			if ($arOption[3][0] == "checkbox" && $val != "Y")
				$val = "N";
			COption::SetOptionString("ialex.importxml", $name, $val, $arOption[1]);
		}
	}
...
<form method="post" action="<? echo $APPLICATION->GetCurPage() ?>?mid=<?= urlencode($mid) ?>&amp;lang=<? echo LANGUAGE_ID ?>">
	...
	<?
	foreach ($arAllOptions as $arOption):
		$val = COption::GetOptionString("ialex.importxml", $arOption[0], $arOption[2]);
		$type = $arOption[3];
	?>
		<tr>
